<script setup lang="ts">
	import { getMemberOrderPreNowAPI, getMemberOrderPrePayAPI } from '@/services/order';
	...
	const getMemberOrderPreData = async () => {
		...
		if (query.count && query.skuId) {
			...
			console.log("getMemberOrderPreNowAPI--> res:", res);
			orderPre.value = res.result
		} else {
			// 调用预付订单 API
			const res = await getMemberOrderPrePayAPI()
			console.log("getMemberOrderPrePayAPI--> res:", res);
			orderPre.value = res.result
		}
	}
	onLoad(async () => {
		await Promise.all([getMemberOrderPreData()])
	})
...
<template>
	<scroll-view scroll-y class="viewport">
		<!-- 收货地址  orderPrePay!.userAddresses.length-->
		<navigator v-if="selecteAddress" class="shipment" hover-class="none" url="/pagesMember/address/address?from=order">
			...
			<view class="address"> {{ selecteAddress.fullLocation}} </view>
